...
/**
 * Generates flashcards from a RAG context.
 *
 * @param {string} context  - Retrieved chunks joined into a string
...
export async function summariseNote(context, style = "detailed") {
  const styleGuide = {
    brief: "Write a single paragraph (3–5 sentences) covering only the most critical points.",
    detailed: "Write a structured summary with an introductory sentence, 3–5 key sections as short paragraphs, and a closing takeaway.",
    bullets: "Return a bullet-point list (use '-') of the 7–10 most important points. Each bullet must be a complete sentence.",
  };
...
/**
 * Extracts key concepts/terms from the note for a glossary or mindmap.
 *
 * @param {string} context
 * @returns {Array<{term: string, definition: string}>}
 */
export async function extractKeyConcepts(context) {
  const prompt = `
Extract all important terms, concepts, or jargon from the content below.
For each, provide a clear, concise definition (1-2 sentences) based solely on the content.
...
Return at most 15 concepts. Prioritise subject-specific vocabulary.
`;
  const result = await generate(prompt);
  if (!result.concepts || !Array.isArray(result.concepts)) {
    throw new ApiError(500, "Malformed concepts response from AI");
  }
