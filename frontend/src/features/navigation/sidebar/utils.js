...
export function childrenIds(folderId, Notes, Folders) {
  ...
  idArray = idArray.concat(
    Notes.filter((element) => element.folderId === folderId)?.map(
      (node) => node.id,
    ),
  );
  Folders.filter((element) => element.parentFolderId === folderId)?.forEach(
    (element) => {
      idArray = idArray.concat(childrenIds(element.id, Notes, Folders));
    },
  );
  return idArray;
}
